...
function generarWhatsAppServicio($servicio) {
    $usuario = isset($_SESSION['usuario']) ? $_SESSION['usuario'] : "cliente";
    $mensaje = "Hola, soy $usuario y estoy interesado en el servicio de $servicio. Me gustaría recibir más información.";
...
<section class="alquiler-section servicios-bg">
    <h2>Nuestros Servicios</h2>
    <p>Ofrecemos soluciones completas para maquinaria pesada.</p>

    <div class="maquinaria-grid">

        <div class="card">
            <h3>Mantenimiento</h3>
            <p>Servicio técnico preventivo y correctivo.</p>
            <?php if(isset($_SESSION['usuario'])): ?>
                <a href="<?php echo generarWhatsAppServicio('Mantenimiento'); ?>" target="_blank"><button>Solicitar servicio</button></a>
            <?php else: ?>
                <button onclick="abrirModal('login')">Solicitar servicio</button>
            <?php endif; ?>
        </div>

        <div class="card">
            <h3>Transporte</h3>
            <p>Traslado seguro de maquinaria.</p>
            <?php if(isset($_SESSION['usuario'])): ?>
                <a href="<?php echo generarWhatsAppServicio('Transporte'); ?>" target="_blank"><button>Solicitar servicio</button></a>
            <?php else: ?>
                <button onclick="abrirModal('login')">Solicitar servicio</button>
            <?php endif; ?>
        </div>
        ...
        <div class="card">
            ...
            <p>Consultoría especializada en maquinaria.</p>
            <?php if(isset($_SESSION['usuario'])): ?>
                <a href="<?php echo generarWhatsAppServicio('Asesoría Técnica'); ?>" target="_blank"><button>Solicitar servicio</button></a>
            <?php else: ?>
                <button onclick="abrirModal('login')">Solicitar servicio</button>
            <?php endif; ?>
        </div>
        ...
        <div class="card">
            <h3>Operadores</h3>
            <p>Personal calificado para maquinaria pesada.</p>
            <?php if(isset($_SESSION['usuario'])): ?>
                <a href="<?php echo generarWhatsAppServicio('Operadores'); ?>" target="_blank"><button>Solicitar servicio</button></a>
            <?php else: ?>
                <button onclick="abrirModal('login')">Solicitar servicio</button>
            <?php endif; ?>
        </div>
    ...
    <div class="modal-content">
        ...
        <div id="formLogin">
            ...
            <form action="../procesar_login.php" method="POST">
                <input type="hidden" name="redirect" value="pages/servicios.php">
